...
// import { transcode, hahahah } from "@poco/codec";
import { hahahah } from "@poco/codec";
  ...
  async takeJob({ jobId }: PocoTakeJobOptions) {
    ...
    rtcConnection.on("SendJob", async (jobIdInString, buffer) => {
      ...
      console.log(buffer);
       ...
       //这里进行转码处理，首先要对buffer进行处理
      //  const result = new File(buffer, "test.mp4")
      //这里的buffer似乎有问题？？？
      console.log('finalbuffer start')
      ...
      //不知道这里会不会有影响？？？要求传入的是uint8array       @ts-ignore 
